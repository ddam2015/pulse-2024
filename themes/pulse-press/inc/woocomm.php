...
function pulse_cart_process(){
  ...
  if( !empty(WC()->cart->get_cart()) ) {
    ...
    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
      ...
      $extra_types = array( 'fee', 'gear', 'clinics', 'tryouts', 'beach', 'beach_tryouts', 'yvl', 'club', 'summerprog', 'parentInfo', 'boysclub' );
...
add_action( 'woocommerce_admin_order_data_after_billing_address_export_csv', 'pulse_player_checkout_field_display_admin_order_meta_export_csv', 10, 1 );
function pulse_player_checkout_field_display_admin_order_meta_export_csv($order){
  //parse the field names, if we have them, then echo them
  $fields = get_post_meta( $order->id, 'pulse_extra_data', true );
...
//     custom start
    ...
    foreach($post_ids as $order){
      $order_id = wc_get_order($order);  // Get the order object
      $total_paid = $order_id->get_total(); // Get the total amount paid for the order
      
      $fields = get_post_meta( $order, 'pulse_extra_data', true );
        ...
        foreach( $fields as $dex => $key ) $fieldscounter++;
         ...
         for( $titlecount = 0, $valuecount = 0; $titlecount < $fieldscounter; $valuecount < $fieldscounter ) {
           ...
           for($j = 0; $valuecount < $fieldscounter && $j < 6; $valuecount++, $j++){
              ...
              unset($key_parts[0]);
              array_pop($key_parts);
              array_push($playerVar, get_post_meta( $order, $fields[$valuecount], true ));
            }
        }
      }
      
      array_push($titleVar, "Total Paid");  // Add "Total Paid" as a title
      array_push($playerVar, $total_paid);  // Add the total paid value
      
      array_push( $titleVarFinale, $titleVar);
      array_push( $playerVarFinale, $playerVar);
